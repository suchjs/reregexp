    ...
    Parser.prototype.parse = function () {
        ...
        var addToGroup = function (cur) {
            if (groups.length || lookarounds.length) {
                var isGroup = (nestQueues.length && getLastItem(nestQueues).type === 'group') || groups.length > 0;
                            ...
                            if (no.charAt(0) === '0') {
                                if (no.length === 1) {
                                    ...
                                    if (+no.charAt(1) > 7) {
                                        ...
                                        var octal = no.length >= 3 && +no.charAt(2) <= 7 ? no.slice(1, 2) : no.charAt(1);
        ...
        if (refOrNumbers.length) {
            var replace_1 = function (lists, search, rep) {
                ...
                if (finded) {
                    lists.splice.apply(lists, [idx, 1].concat(rep));
                }
            };
        ...
        if (queues.length > 1 && queues[1].type === 'group' && queues[1].isRoot === true) {
    ...
    RegexpSet.prototype.prebuild = function (conf) {
        ...
        if (this.reverse) {
            if (!this.codePointResult) {
                if (queues.length === 1 && queues[0].type === 'charset' && ['w', 's', 'd'].indexOf(queues[0].charset.toLowerCase()) > -1) {
